...
class GastoAdapter(
    ...
    private val onItemClick: (GastoConCategoria) -> Unit,
    private val onEditClick: (GastoConCategoria) -> Unit
) : RecyclerView.Adapter<GastoAdapter.GastoViewHolder>() {

    inner class GastoViewHolder(val binding: ItemExpenseBinding) :
    ...
    override fun onBindViewHolder(holder: GastoViewHolder, position: Int) {
        ...
        holder.binding.tvCategoria.text = "${item.categoriaIcono} ${item.categoriaNombre}"
        holder.binding.tvMonto.text = String.format("-$%.2f", item.monto)
        holder.binding.tvFecha.text = item.fecha
        ...
        holder.binding.btnEditar.setOnClickListener { onEditClick(item) }
    }
}
